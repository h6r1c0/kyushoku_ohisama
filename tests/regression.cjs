...
(async()=>{
 ...
 change(a,'drySnack',true);assert.equal(a.d.getElementById('dryPeople').readOnly,true);assert.equal(Number(a.d.getElementById('dryPeople').value),a.ev('snackPeopleTotal()'));assert.equal(a.d.getElementById('dryResultNote').textContent,'1人量が未設定です');
 ...
 a.ev('renderHelpGuides()');await new Promise(r=>setTimeout(r,60));assert.equal(a.errors.length,0,a.errors.map(e=>e.message).join('\n'));const ids=[...a.d.querySelectorAll('[id]')].map(e=>e.id);assert.equal(ids.length,new Set(ids).size);assert.ok(a.d.getElementById('helpCutLive').textContent.includes('分数計算'));assert.ok(a.d.getElementById('helpCutLive').textContent.includes('おやつ人数'));assert.ok(a.d.getElementById('helpDryLive').textContent.includes('少なめ8g'));assert.ok(a.d.querySelectorAll('.help-caption.guide-operation').length>0);assert.ok(a.d.querySelectorAll('.help-caption.guide-info').length>0);a.close();
 ...
 assert.equal(a.ev('foods.some(f=>f.id==="teba"||f.id==="sasami"||f.id==="buri")'),false);assert.equal(a.ev('foods.filter(f=>f.id==="akauo").length'),1);
